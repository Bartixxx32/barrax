...
class BanRole():
    ...
    async def nickrole(self, ctx, nickname, *, role: str):
        """Nick all members with the specified role.
        Make sure the bot's role is higher in the role
        hierarchy than the role you want it to nick"""
        ...
        msg = await self.bot.say("{}/{} members nicked".format(ban_count, total_count))
        for member in members_to_ban:
            try:
                ...
                ban_count += 1
                msg = await self.bot.edit_message(msg, "{}/{} members nicked".format(ban_count, total_count))
        ...
        await self.bot.say("Done nicking members")
